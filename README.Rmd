...
[![Travis-CI Build Status](https://travis-ci.org/jaydorsey/ghgvcR.svg?branch=master)](https://travis-ci.org/jaydorsey/ghgvcR)
...
For Calculator development, use Docker-compose.

#### Using Docker Compose

Docker Compose is the easiest way to develop & use the Calculator locally.

```
> docker-compose build && docker-compose up
```

The first time you run these commands the container will take a few minutes to
start up. This is because it's downloading and caching the map files in a
persistent volume.


#### Using Docker

Use Docker Compose (above) unless you need to modify the Docker image itself.


##### Building and tagging the image

```
> docker build . --rm --force-rm -t jaydorsey/ghgvcr
```
...
```
> docker run -it --rm -v ~/data:/app/data -p 6311:6311 --name ghgvcr-serve jaydorsey/ghgvcr
```

You'll want to mount the `/app/data` folder so it persists across container
restarts

#### Running R tests for local development

```
# Create a container (or connect to your existing)
> docker-compose build && docker-compose run ghgvcr /bin/bash
