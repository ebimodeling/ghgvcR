...
now check it out:
```{r echo=FALSE, comment=FALSE, results='hide', include=FALSE}
options(warn=FALSE)
# test("ghgvcR")
# example(ghgvcR)
library(devtools)
...
ggplot(data = forcings.long, aes(x = factor(variable), y = value, fill = ecosystem)) + geom_bar(position="dodge") + ggtitle(label="Example plot: values of F for two ecosystems") + xlab("Variable") + ylab("Units of F") + coord_flip()
