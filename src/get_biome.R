...
if (length(args) != 6) stop("Incorrect number of arguments.\n
                            USAGE: 'get_biome.R ghghv_dir run_dir out_dir netcdf_dir latitude longitude'
                            ghgvc_dir:  directory of the ghgvc app
                            run_dir:    directory of the ghgvcR program
                            out_dir:    directory to save the biome data to
                            netcdf_dir: directory containing the biome data
...
print(head(res))
write_json(toJSON(res), out_dir, "biome", format = c("json"))

#Get the named ecosystem data
name_indexed_ecosystems = fromJSON(file(paste0(ghgvc_dir, "/public/data/final_ecosystems.json")))
...
write_json(toJSON(biome_data), out_dir, "biome", format = c("json"))
