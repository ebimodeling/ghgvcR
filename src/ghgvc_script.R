...
storage.plot <- baseplot + 
  geom_bar(data = plotdata, aes(x = Biome, y = Storage), fill = "DarkGreen", width = 0.25, stat = "identity")
ongoing.plot <- baseplot +
  geom_bar(data = plotdata, aes(x = Biome, y = Ongoing_Exchange), fill = "DarkBlue", width = 0.25, stat = "identity")
crv.plot <- baseplot +
  geom_bar(data = plotdata, aes(x = Biome, y = CRV_BGC), fill = "Grey", width = 0.25, stat = "identity")

grid.arrange(arrangeGrob(ylabels, storage.plot, ongoing.plot, crv.plot, ncol = 4,
                         #main = "",
                         #sub = textGrob(xlabels, hjust = 0.3)
